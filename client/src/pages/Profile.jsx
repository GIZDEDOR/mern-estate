...
export default function Profile() {
  ...
  const handleShowListings = async () => {
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        setShowListingsError(errorData.message);
        return;
      }
      const data = await res.json();
    ...
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <p className="text-sm self-center">
          {fileUploadError ? (
            <span className="text-red-700">Ошибка загрузки изображения (Изображение должно вестить не больше 10 мб)</span>
      ...
      <button onClick={handleShowListings} className="text-green-700 w-full">Показать Объекты</button>
      <p className="text-red-700 mt-5">{showListingsError ? 'Ошибка просмотра объявлений' : ''}</p>

      {userListings && 
        userListings.length > 0 &&
        <div className="flex flex-col gap-4">
          <h1 className="text-center mt-7 text-2xl font-semibold">Мои Объекты</h1>
          {userListings.map ((listing) => (
            ...
            >
              ...
              <button className="text-red-700 uppercase">Удалить</button>
